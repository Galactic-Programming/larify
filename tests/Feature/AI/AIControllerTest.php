...
describe('POST /api/ai/projects/{project}/chat', function () {
    it('allows project member to chat with AI', function () {
        $user = User::factory()->create();
        $project = Project::factory()->create(['user_id' => $user->id]);

        mockGeminiService([
            'chat' => 'Hello! I can help you manage your project. You have 5 pending tasks.',
        ]);

        $response = $this->withoutMiddleware(EnsureUserCanUseAI::class)
            ->actingAs($user)
            ->postJson(route('api.ai.projects.chat', $project), [
    ...
    it('prevents non-members from chatting', function () {
        $owner = User::factory()->create();
        $outsider = User::factory()->create();
        $project = Project::factory()->create(['user_id' => $owner->id]);
    ...
    it('returns 422 when AI fails to respond', function () {
        $user = User::factory()->create();
        $project = Project::factory()->create(['user_id' => $user->id]);

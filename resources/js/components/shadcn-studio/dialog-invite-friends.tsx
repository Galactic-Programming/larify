    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <DialogContent className={cn('sm:max-w-lg', className)}>
                ...
                <form onSubmit={handleEmailSubmit} className="flex gap-4 max-sm:flex-col">
                                    ...
                                    <div className="flex items-center gap-3 max-[420px]:w-50">
                                        <Avatar className="size-10">
                                            ...
                                            <AvatarFallback className="text-xs">
                                                {person.avatarFallback || getInitials(person.name)}
                                            </AvatarFallback>
                                        </Avatar>
                                        <div className="flex flex-1 flex-col overflow-hidden">
                                            <span>{person.name}</span>
                                            <span className="text-muted-foreground truncate text-sm">{person.email}</span>
                                        </div>
                                    </div>
                                    <Button
                                        size="sm"
                                        onClick={() => handlePersonInvite(person)}

import { LucideIcon, CircleAlertIcon, CheckCheckIcon, TriangleAlertIcon, InfoIcon } from 'lucide-react'
...
const defaultIcons: Record<NonNullable<VariantProps<typeof alertSoftVariants>['variant']>, LucideIcon> = {
...
const AlertSoft = ({ variant = 'default', title, description, icon, className }: AlertSoftProps) => {
    ...
    return (
        <Alert className={cn(alertSoftVariants({ variant }), className)}>
            ...
            {description && <AlertDescription className={descriptionVariants({ variant })}>{description}</AlertDescription>}
        </Alert>
    )
}
